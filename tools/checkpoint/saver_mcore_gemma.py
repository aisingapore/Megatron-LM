...
class MCoreTESetter(MCoreSetter):
    ...
    def set_layer(
        ...
        cls.set_tensor(l.post_self_attn_layernorm.weight, post_attention_norm_weight)
        # MLP.
        ...
        cls.set_tensor(l.post_mlp_layernorm.weight, post_mlp_layernorm_weight)
...
def save_checkpoint(queue, args):
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer_id in range(len(setter.get_transformer_block(models[pp_rank][0][0]).layers)):
            ...
            post_norm_weight = msg.pop("pre mlp norm")
            # for gemma
            post_attention_layernorm_weight = msg.pop("post attn norm weight")
            post_mlp_layernorm_weight = msg.pop("post mlp norm")
            ...
            for ep_rank in range(args.target_expert_parallel_size):
                for tp_rank in range(args.target_tensor_parallel_size):
                    params_dict = {
                        ...
                        "post_attention_norm_weight": post_attention_layernorm_weight,
                        "post_mlp_layernorm_weight" : post_mlp_layernorm_weight
