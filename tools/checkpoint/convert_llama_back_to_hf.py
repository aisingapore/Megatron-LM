...
class CheckpointLoader:
    ...
    def _load_checkpoints(self) -> List[List]:
        """
    ...
    def load_model(self, from_model = None, from_pretrained= None, dtype = "bfloat16") -> AutoModelForCausalLM:
        ...
        # Empty out the state dict for verification
...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert from Megatron-LM Llama to Huggingface")
    parser.add_argument("base_folder", type=str,
                        help="Path to the folder containing the Llama checkpoint")
    ...
    parser.add_argument("--reference-hf-model", type=str, required=True,
                        help="Path to the reference hf model, should contain config.json etc. This is called by `from_pretrained`")
...
def main():
    ...
    python convert_llama_back_to_hf.py --pp-size 1 --tp-size 1 --reference-hf-model /path/to/hf/model --save-path /path/to/save/checkpoint
        --megatron-dir /path/to/megatron-lm --dtype bfloat16 /path/to/megatron_checkpoint/iter000
